...
class Model_DRN(object):
    ...
    def build_generator(self):
        print("[DRN Model] Building Makeup Encoder....")
        src_image = tf.keras.layers.Input(self.input_shape)
        src_x = src_image
        ref_image = tf.keras.layers.Input(self.input_shape)
        ref_x = ref_image

        # INPUT LAYER
        src_x = Conv2D_layer(src_x, filters = 64, kernel_size = (7, 7))
        src_x = ReLU_layer(src_x)
        ref_x = Conv2D_layer(ref_x, filters = 64, kernel_size = (7, 7))
        ref_x = ReLU_layer(ref_x)

        # CONCATENATE, CONVOLUTION
        x = Concatenate_layer(src_x, ref_x, axis=1)
        ...
        model = tf.keras.Model(inputs = [src_image, ref_image], outputs = image)
        return model
    
    def build_discriminator(self):
        print("[DRN Model] Building Discriminator....")
    ...
    def build(self, isLoadWeight = False):
        print("[DRN Model] Building Train Model....")
        # Training flow
    ...
    def export_model(self, save_model_path, export_path):
        print("[DRN Model] Exporting Model....")
    ...
    def save_model(self, model, model_path):
        ...
        print('[DRN Model] Save weights to {}.'.format(model_path))

    def load_model(self, model, model_path):
        model.load_weights(model_path)
        print('[DRN Model] Load weights from {}.'.format(model_path))