...
class Model_BG(object):
    ...
    def loss_function(self, pred, labels, dis_result):
        ...
        return loss, [adversarial_loss, cycle_consistency_loss, perceptual_loss, makeup_loss, background_loss], [source_background, transfer_background], predicted
    ...
    def train_step(self, features, labels):
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tapeX, tf.GradientTape() as disc_tapeY:
            ...
            gen_loss, loss_list, bg_images, predicted_seg = self.loss_function(pred, labels, [fake_source, fake_reference])
        ...
        return gen_loss, dis_loss_X, dis_loss_Y, pred["image"], bg_images, predicted_seg, loss_list
    ...
    def train(self, train_dataset, epochs = 500, pretrained_model_path = None):
        ...
        for epoch in range(epochs):
            ...
            for batch_features, batch_labels in dataset:
                ...
                gen_loss, dis_loss_X, dis_loss_Y, transfer_image, bg_images, predicted_seg, loss_list = self.train_step(batch_features, batch_labels)
                step += 1
                if(step % 10 == 0):
                    ...
                    save_images(epoch_num, step, self.batch_size, [batch_features["images1"].numpy(), bg_images[0].numpy(), bg_images[1].numpy()],  self.bg_save_path)
                    save_images(epoch_num, step, self.batch_size, [batch_labels["face_true"].numpy(), batch_labels["brow_true"].numpy(), batch_labels["eye_true"].numpy(), batch_labels["lip_true"].numpy(), predicted_seg[0].numpy(), predicted_seg[1].numpy(),predicted_seg[2].numpy(),predicted_seg[3].numpy()], self.gt_save_path)
                if(step % 200 == 0):
                    save_images(epoch_num, step, self.batch_size, [batch_features["images1"].numpy(), transfer_image[0].numpy(), batch_labels["whole_face_true"].numpy(), batch_features["images2"].numpy(), transfer_image[1].numpy()], self.pic_save_path)
